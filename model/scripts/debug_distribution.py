...
def build_sequence(log_data):
    ...
    method = log_data.get('request', {}).get('method', '') if isinstance(log_data.get('request'), dict) else log_data.get('request.method', '')
    path = log_data.get('request', {}).get('path', '') if isinstance(log_data.get('request'), dict) else log_data.get('request.path', '')
    protocol = log_data.get('request', {}).get('protocol', '') if isinstance(log_data.get('request'), dict) else log_data.get('request.protocol', '')
...
def main():
    ...
    if z_score > 3:
        ...
        print(f"    This indicates a distribution mismatch.")
