...
def extract_features(log_texts, tokenizer, model, num_runs=3):
    ...
    for i in range(num_runs):
        encodings = tokenizer(log_texts, padding=True, truncation=True, max_length=MAX_LENGTH, return_tensors="pt").to(
            device
        )
...
def batch_inference(df, tokenizer, model, scaler, iforest, train_stats, batch_size=64):
    ...
    for i in tqdm(range(0, len(logs), batch_size), desc="Batch inference", ncols=100):
        ...
        for j in range(len(batch_logs)):
            category, combined, z, if_score, _ = predict_anomaly(err[j], cls[j], perp[j], scaler, iforest, train_stats)
...
if __name__ == "__main__":
    ...
    print(
        f"Training Stats → mean={train_stats['mean_error']:.4f}, std={train_stats['std_error']:.4f}, 95th={train_stats['threshold_percentile']:.4f}"
    )

    test_df = pd.read_csv(TEST_CSV).fillna("").astype(str)
    preds, scores, errors, zscores = batch_inference(
        test_df, tokenizer, model, scaler, iforest, train_stats, batch_size=BATCH_SIZE
    )
