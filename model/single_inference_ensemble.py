...
def main():
    ...
    category, details = predict_anomaly(
        reconstruction_error, cls_embedding, perplexity, scaler, iforest, train_stats
    )
    ...
    print(f"  [{'✓' if details['statistical_anomaly'] else '✗'}] Statistical (z-score: {details['z_score']:.2f} {'> 3' if details['statistical_anomaly'] else '< 3'})")
    print(f"  [{'✓' if details['percentile_anomaly'] else '✗'}] Percentile (loss: {reconstruction_error:.4f} {'>' if details['percentile_anomaly'] else '<'} {train_stats['threshold_percentile']:.4f})")
